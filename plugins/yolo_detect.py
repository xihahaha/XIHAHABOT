...
async def yolo(session: CommandSession):
    msg = session.get('message', prompt='使用官方YOLOv5x训练模型，请发送图片')
    p = '\\[CQ\\:image\\,file\\=.*?\\,url\\=(.*?)\\]'
    img_url = re.findall(p, msg)
    img_s = str(img_url).replace('[', '').replace(']', '').replace("'", "")
    img = requests.get(img_s)
    open('./inference/input/yolo.jpg', 'wb').write(img.content)
    detect()
    pic = await get_pic()
...
def detect(save_img=False):
    ...
    parser.add_argument('--output', type=str, default='C:/Users/Administrator/Desktop/CQPro/data/image/AI', help='output folder')  # output folder
